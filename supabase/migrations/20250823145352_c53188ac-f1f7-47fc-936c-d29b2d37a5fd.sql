...
-- Create trigger for automatic parent path calculation
CREATE TRIGGER IF NOT EXISTS trg_sections_parent_fill
BEFORE INSERT OR UPDATE ON public.sections
FOR EACH ROW EXECUTE FUNCTION public.sections_parent_fill();

-- Create enhanced chunk citation view
CREATE OR REPLACE VIEW public.chunk_citations AS
SELECT 
  c.id as chunk_id,
  d.insurer,
  (d.product_name || COALESCE(' ' || d.version, '')) as polis,
  s.section_path,
  ('art. ' || REPLACE(s.section_path::text, '.', '.')) as article_label,
  s.title as section_title,
  (d.insurer || ' ' || (d.product_name || COALESCE(' ' || d.version, '')) ||
   ' — art. ' || REPLACE(s.section_path::text, '.', '.') || ' ' ||
   COALESCE(s.title, '')
  ) as citation_label
